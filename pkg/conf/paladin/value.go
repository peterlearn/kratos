...
func NewValue(val interface{}, raw []byte) *Value {
	return &Value{
		val: val,
		raw: raw,
	}
...
func (v *Value) Bool() (bool, error) {
	if v.val == nil {
		log.Error("value key not exist")
		return false, ErrNotExist
	}
	b, ok := v.val.(bool)
	if !ok {
		log.Error("value type assertion no match")
...
func (v *Value) Slice(dst interface{}) error {
	...
	if !ok {
		log.Error("value different types")
		return ErrDifferentTypes
	}
	for _, s := range src {
...
func (v *Value) UnmarshalTOML(dst interface{}) error {
	...
	return toml.Unmarshal(text, dst)
}
...
func (v *Value) UnmarshalJSON(dst interface{}) error {
	...
	return json.Unmarshal(text, dst)
}
...
func (v *Value) UnmarshalYAML(dst interface{}) error {
	...
	return yaml.Unmarshal(text, dst)
}
