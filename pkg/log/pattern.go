...
var patternMap = map[string]func(map[string]interface{}) string{
	...
	"z": keyFactory(_zone),
	"q": keyFactory(KeyTS),
	"a": keyFactory(KeyPath),
	"r": keyFactory(KeyRet),
	"l": keyFactory(KeyReqLen),
	"u": keyFactory(KeyRspLen),
	"c": keyFactory(KeyUser),
	"S": longSource,
	"s": shortSource,
	"M": message,
}

// max log req rsp str length
const MaxLogMsgLength = 500
...
func isInternalKey(k string) bool {
	switch k {
	case _level, _levelValue, _time, _source, _instanceID, _appID, _deplyEnv, _zone, KeyTS, KeyPath, KeyReqLen, KeyRspLen, KeyRet, KeyUser:
		return true
	}
	return false
}

func isKeyPair(k string) bool {
	switch k {
	case KeyTS, KeyPath, KeyReqLen, KeyRspLen, KeyRet, KeyUser:
		return true
	}
	return false
}
